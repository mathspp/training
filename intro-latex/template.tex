...
\usepackage[utf8]{inputenc} % For latin accented letters like ãáà
\usepackage{verbatim}       % To type weird inputs, like commands
\usepackage{hyperref}       % For automatic links and
                            % clickable references to equations, sections, etc
\usepackage{amsfonts}       % For things like \mathbb{R} and \mathcal{L}
\usepackage{amsmath}        % For things like \implies and \iff
\usepackage{float}          % To force LaTeX to place images/tables where we tell it to
\usepackage{graphicx}       % Include images in the figure environment
\usepackage{color}          % Para poderes escrever texto com outras cores
